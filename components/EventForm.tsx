...
function EventForm({ mode, initialData }: EventFormProps) {
    ...
    const generateUploadUrl = useMutation(api.storage.generatedUploadUrl)
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <FormField
           ...
           <div className="space-y-4">
            ...
            text-gray-700">Event Image</label>
            <div className="mt-1 flex items-center gap-4">
              {imagePreview || (!removedCurrentImage && currentImageUrl) ? (
                <div className="relative w-32 aspect-square bg-gray-100 rounded-lg ">
                  ...
                  text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 transition-colors"></button>
                </div>
              ): (
                <input type="file"
                accept="image/*"
                onChange={handleImageChange}
                ref={imageInput} className="block w-full text-sm text-gray-500
          ...
          <Button type="submit"
          disabled={isPending}
          className="w-full bg-gradient from-blue-600 to-blue-800 
